      ...
      eventlist.push(
        <Card key={keyvalue} className="w-[18vw] mb-[1vw]">
          <CardHeader>
            ...
            <Label>
              {DateTime.fromISO(eventarray[i]!.start_time).toLocaleString(
                DateTime.DATE_MED
              )}{' '}
              to{' '}
              {DateTime.fromISO(eventarray[i]!.end_time).toLocaleString(
                DateTime.DATE_MED
              )}
            </Label>
          </CardHeader>
          ...
          <CardFooter className="flex justify-between">
            <Button
              onClick={() => handleClick(eventarray[i]!._id)}
              className="h-6 rounded-md px-3 m-1"
            >
              Delete Event
            </Button>
          </CardFooter>
